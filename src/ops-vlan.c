...
get_vlan_data(int vid, bool internal)
{
    ...
    vlanp->hw_created = 0;
    VLOG_INFO("Sourav : get_vlan_data : user_created to = %d\n", vlanp->user_created);
    if (vlanp->user_created != 1) {
        vlanp->user_created = 0;
    }
...
bool is_user_created_vlan(int vid)
{
    ...
    if (vlanp) {
        return (vlanp->user_created) ? true : false;
    }
    return false;
}

void set_created_by_user(int vid, int status)
{
    ops_vlan_data_t *vlanp = ops_vlans[vid];
    vlanp = ops_vlans[vid];
