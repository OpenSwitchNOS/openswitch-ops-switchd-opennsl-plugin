...
int bcmsdk_get_l3_ingress_stats(struct ops_deleted_stats *del_stats,
                               ...
                               uint32_t ingress_num_id)
{
    ...
    rc = opennsl_l3_ingress_stat_counter_get(0, ingress_vlan_id,
...
#if 0
    /* Get the drop packet stats */
    /* Initialize the stat structures for fetching packet details*/
    memset(counter_index, 0 , 10);
    opennsl_stat_value_t_init(&(count_arr[0]));
    VLOG_DBG("opennsl_stat_init drop packets SUCCESS for l3 ing vlan id: %d",
              ingress_vlan_id);

    rc = opennsl_l3_ingress_stat_counter_get(0, ingress_vlan_id,
                                       opennslL3StatDropPackets, ingress_num_id,
                                       &(counter_index[0]),
                                       &(count_arr[0]));
    if (rc) {
        VLOG_ERR("Failed to get stat input drop pkt for l3 ingress vlan id: %d",
                  ingress_vlan_id);
        return 1; /* Return error */
    }
    stats->rx_dropped = count_arr[0].packets;
    VLOG_DBG(" stats->rx_dropped = %" PRIu64, stats->rx_dropped);
#endif

    return rc;
}
int
bcmsdk_get_l3_egress_stats(struct ops_deleted_stats *del_stats,
                    ...
                    uint32_t egress_num_id)
{
    ...
    rc = opennsl_l3_egress_stat_counter_get(0, egress_object_id,
    ...
    memset(counter_index, 0 , 10);
    opennsl_stat_value_t_init(&(count_arr[0]));
    VLOG_DBG("opennsl_stat_init drop packets SUCCESS for l3 egress id: %d",
              egress_object_id);
    ...
    rc = opennsl_l3_egress_stat_counter_get(0, egress_object_id,
                                      ...
                                      &(count_arr[0]));
    if (rc) {
        VLOG_ERR("Failed to get stat output drop packets for l3 egress id: %d",
                 egress_object_id);
        return 1; /* Return error */
    }
    stats->tx_dropped = count_arr[0].packets + del_stats->del_drop_packets;
    VLOG_DBG(" stats->tx_dropped = %" PRIu64, stats->tx_dropped);
#endif
