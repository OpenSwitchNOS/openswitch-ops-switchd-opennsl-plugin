...
int ops_copp_unknown_ip_unicast (uint32 unit)
{
    ...
    /* Program the rx rule to send packets with priroty COPP_MAX_PACKET_COS to unknown IP cpuq */
    int_prio = COPP_MAX_PACKET_COS;
    int_prio_mask = 0xff;
    ...
    if (OPENNSL_FAILURE(retval)) {
        VLOG_ERR("     Packet class: Failed to program the packet"
                 "class rule for unknown IP packets with max priority %s\n",
                 opennsl_errmsg(retval));
        return(OPS_COPP_FAILURE_CODE);
    }
