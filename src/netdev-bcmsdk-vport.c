...
check_route(struct netdev_vport *netdev)
{
    ovs_be32 route;
    int l3_egress_id;
    /* TODO with ipv6 checking */
    route = in6_addr_get_mapped_ipv4(&netdev->tnl_cfg.ipv6_dst);
    if (ops_egress_lookup_from_dst_ip(netdev->carrier.vrf,
        route, &l3_egress_id)) {
...
set_tunnel_config(struct netdev *dev_, const struct smap *args)
{
    ...
    if(tnl_cfg.in_key_present)
        VLOG_DBG("set_tunnel_config key %lx",(unsigned long int)ntohll(tnl_cfg.in_key));
...
int
netdev_bcmsdk_vport_set_tunnel_id(struct netdev *netdev, int tunnel_id)
{
    struct netdev_vport *dev = netdev_vport_cast(netdev);
    ovs_mutex_lock(&dev->mutex);
    dev->vport.tunnel_id = tunnel_id;
    /* Below is temporaty, to be fixed */
...
int
netdev_bcmsdk_vport_get_vport_id(char *port)
{
    int vxlan_port_id = -1;
    ...
    if(port) {
        ...
        if(netdev_ && is_vport_class(netdev_get_class(netdev_)))
        {
            ...
            ovs_mutex_unlock(&dev->mutex);
            VLOG_DBG("Got vxlan port id 0x%x for port %s", vxlan_port_id, port);
            netdev_close(netdev_);
        }
    }
    return vxlan_port_id;
}
void
netdev_bcmsdk_vport_set_tunnel_vport(struct netdev *netdev, bcmsdk_vxlan_port_t * vport)
{
    struct netdev_vport *dev  = netdev_vport_cast(netdev);
    if(vport) {
    ...
    OVS_REQUIRES(netdev->mutex)
{
    ...
    if(!VALID_PORT(dev->carrier.port)) {
        ...
        status = netdevv_bcmsdk_get_link_status(dev->hw_unit, dev->carrier.port);
 ...
 * the neighbor mac if it dest IP matches host.
    ...
    SHASH_FOR_EACH (node, &device_shash) {
        ...
        if(netdev) {
            ...
            if(dev && (TUNNEL_ACTION(event, dev->tnl_state))) {
                ...
                /* If destination IP match host,it's neighbor */
    ...
    if(TUNNEL_BIND(event, dev->tnl_state)) {
        ...
        if(ops_egress_lookup_from_route(dev->carrier.vrf, route_prefix, &l3_egr_id)) {
            if(set_tunnel_nexthop(dev, l3_egr_id)) {
                ...
                ipv4 =  in6_addr_get_mapped_ipv4(&dev->tnl_cfg.ipv6_dst);
                /* Temporary sending ping.  Arpmgrd will take care of it */
                do_ping(&ipv4); /* To be removed */
            }
        }
...
tunnel_print(struct ds *ds, const struct netdev_tunnel_config *tnl_cfg)
{
    ...
    ovs_be32 ipv4_dst;
    ovs_be32 ipv4_src;  //temp for priintint
    if (!tnl_cfg) {
    ...
    if(netdev_ && is_vport_class(netdev_get_class(netdev_)))
    {
        ...
        if(dev) {
            ...
            ds_put_format(ds, "port %d, intf id %d vlan %d\n",dev->carrier.port,
                         dev->vport.l3_intf_id, dev->vport.l3_intf_id);
...
{
    netdev_vport_dump();
    unixctl_command_reply(conn,"Dump done\n");
}
...
static void
diag_do_ping(struct unixctl_conn *conn, int argc,
        const char *argv[], void *aux OVS_UNUSED)
{
    struct ds ds = DS_EMPTY_INITIALIZER;
    uint32_t ip;
    struct in_addr in_addr;
    if (!lookup_ip(argv[1], &in_addr)) {
        ip = ntohl(in_addr.s_addr);
        do_ping(&ip);
        ds_put_format(&ds, "Ping Ip 0x%x\n", ip);
    } else {
        ds_put_format(&ds, "Invalid Ip address %s",argv[1]);
    }
    unixctl_command_reply(conn, ds_cstr(&ds));
    ds_destroy(&ds);
}
...
void netdev_bcmsdk_vport_register(void) {
    ...
    unixctl_command_register("tnl/doping", "ip", 1, 1,
                             diag_do_ping, NULL);
    /* Setting next hop from terminal for debug */
    unixctl_command_register("tnl/nexthop", "[tunnel netport localmac remotemac,"
                             " l3intf_id vlan (optional)]", 5, 6,
