...
ops_vport_lsw_create(int hw_unit, int vni)
{
    ...
    if( (rc = bcmsdk_vxlan_multicast_operation(hw_unit, opcode, &multi_group)) != 0)
        return rc;
...
ops_vport_bind_mac(int hw_unit, char *port, int vni, uint8_t *mac_str)
{
    ...
    rc = bcmsdk_vxlan_logical_switch_operation(hw_unit, BCMSDK_VXLAN_OPCODE_GET,
                                          &logical_sw_p);
    if(rc) {
        ...
        const char *argv[], void *aux OVS_UNUSED)
{
    ...
    if (rc) {
        ...
        ds_put_format(&ds, "Successful create lsw, multicast group %d, vnid %d, vpnid 0x%x\n", multi_group.group_id, lsw.vnid,lsw.vpn_id);
    }
...
ops_vport_delete_tunnel(struct netdev *netdev)
{
    ...
    if(!netdev_bcmsdk_vport_get_tunnel_id(netdev, &tunnel.tunnel_id)
        ...
        rc = bcmsdk_vxlan_tunnel_operation(unit, BCMSDK_VXLAN_OPCODE_DESTROY, &tunnel);
        if(rc) {
...
ops_vport_create_tunnel(struct netdev *netdev)
{
    ...
    if (!ipv6_addr_is_set(&tnl_cfg->ipv6_src)) {
        if((VALID_PORT(carrier->port)) && ofproto_find_ip4_from_port(carrier->port, &tunnel.local_ip)) {
            VLOG_DBG(" Found source IP4: 0x%x for port %d", tunnel.local_ip, carrier->port);
        } else {
            VLOG_ERR("Can't find src IP for port %d", carrier->port);
            return 1;
        }
    } else {
        /* Take it as ipv4 --- TODO must find out if it's IPV4 */
    ...
    rc = bcmsdk_vxlan_tunnel_operation(unit, BCMSDK_VXLAN_OPCODE_CREATE, &tunnel);
    if(!rc) {
...
ops_vport_bind_net_port(struct netdev *netdev)
{
    ...
    if (vport && carrier) {
        if(!is_ready_to_bind(vport, carrier)) {
            VLOG_ERR("Not ready to bind vxlan port, vxlan_port_cfg not valid\n");
            return 1;
        }
        ...
        netdev_bcmsdk_vport_get_hw_info(netdev, &unit, NULL, NULL);
        rc = bcmsdk_vxlan_port_operation(unit, BCMSDK_VXLAN_OPCODE_CREATE,  &vxlan_port);
        if(rc) {
            VLOG_ERR("failed to bind network port:%s\n", opennsl_errmsg(rc));
            ops_vport_dump(&vxlan_port);
            return rc;
        }
        VLOG_INFO("\n--- Successfully binding net port %d to vpn with vxlan port id 0x%x ---\n",
...
ops_vport_unbind_net_port(struct netdev *netdev)
{
    ...
    if(vport && VALID_EGRESS_ID(vport->egr_obj_id) && carrier) {
        ...
        vxlan_port.port = carrier->port;
        netdev_bcmsdk_vport_get_hw_info(netdev, &unit, NULL, NULL);
        rc = bcmsdk_vxlan_port_operation(unit, BCMSDK_VXLAN_OPCODE_DESTROY, &vxlan_port);
        if(rc) {
...
ops_vport_bind_access_port(int hw_unit, opennsl_pbmp_t pbm, int vni, int vlan)
{
    ...
    OPENNSL_PBMP_ITER(pbm, aport) {
        ...
        rc = bcmsdk_vxlan_port_operation(hw_unit, BCMSDK_VXLAN_OPCODE_CREATE, &port);
        if (rc) {
...
ops_vport_unbind_access_port(int hw_unit, opennsl_pbmp_t pbm, int vni)
{
    ...
    OPENNSL_PBMP_ITER(pbm, aport) {
        ...
        if(VALID_VPORT_ID(port.vxlan_port_id)) {
            rc = bcmsdk_vxlan_port_operation(hw_unit, BCMSDK_VXLAN_OPCODE_DESTROY, &port);
            if (rc) {
...
update_tunnel_src_ip(struct netdev *netdev, int *tunnel_id)
{
    ...
    if(netdev && !netdev_bcmsdk_vport_get_tunnel_id(netdev, &old_tnl.tunnel_id)
              && VALID_TUNNEL_ID(old_tnl.tunnel_id)) {

        /* Retrieve current tunnel structure from bcm */
        rc = bcmsdk_vxlan_tunnel_operation(unit, BCMSDK_VXLAN_OPCODE_GET, &old_tnl);
        if(rc) {
        ...
        if(new_tnl_cfg) {
            ...
            if(!ipv6_addr_is_set(&new_tnl_cfg->ipv6_src)) {
                const carrier_t *carrier = netdev_bcmsdk_vport_get_carrier(netdev);
                if(carrier && (VALID_PORT(carrier->port)
                           && ofproto_find_ip4_from_port(carrier->port, &new_src_ip))) {
                    VLOG_DBG(" Found source IP4: 0x%x for port %d", new_src_ip, carrier->port);
                } else {
            ...
            if(old_tnl.local_ip != new_src_ip ) {
                ...
                VLOG_DBG("Update tunnel initiator - How about tunnel terminator?\n");
                /*
                 * TODO
                 *
                rc = vxlan_update_tunnel_initiator(unit, &old_tnl);
                if (rc) {
                    VLOG_ERR("%s - rc: %s", __func__, opennsl_errmsg(rc));
                    return rc;
                }
                *tunnel_id = old_tnl.tunnel_id;
                */
                return 0;
            }
        }
...
ops_vport_update_tunnel(struct netdev *netdev, int flags)
{
    ...
    if(flags & UPDATE_SRC_IP) {
        ...
        if(!rc) {
            ...
            VLOG_DBG("Successful updating Source IP tunnel id 0x%x\n", tnl_id);
            return 0;
        }
    } else {
    ...
    netdev_change_seq_changed(netdev);  /* Just for compiling */
    return 1;
}
...
{
    if(l2_addr && ds) {
        ds_put_format(ds, "vxlan port 0x%x vpn id %d",l2_addr->port, l2_addr->vid);
