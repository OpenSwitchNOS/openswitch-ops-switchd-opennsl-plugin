...
struct ofproto_mlearn_hmap G_buffer[2] OVS_GUARDED_BY(mlearn_mutex);
...
{
    struct ofproto_mlearn_hmap_node *entry;
    struct eth_addr mac_eth;
    memcpy(mac_eth.ea, mac, sizeof(mac_eth.ea));
    uint32_t hash = mlearn_table_hash_calc(mac_eth, vlan, hw_unit);
    int actual_size = (hmap_entry->buffer).actual_size;
    bool found = false;
    char port_name[16];
    memset((void*)port_name, 0, sizeof(port_name));

    netdev_port_name_from_hw_id(0, port_id, port_name);
