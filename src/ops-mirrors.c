...
/*
 * expand the bundle to its individual ports and obtaining
...
mirror_get_stats (struct ofbundle *mtp, uint64_t *packets, uint64_t *bytes)
{
    ...
    int *hw_units, *hw_ports;
    int i, n;

    hw_units = (int*) xmalloc(LAG_PORT_LIMIT * sizeof(int));
    hw_ports = (int*) xmalloc(LAG_PORT_LIMIT * sizeof(int));
        ...
        struct ofbundle *source, uint32 flags)
{
    ...
    hw_units = (int*) xmalloc(LAG_PORT_LIMIT * sizeof(int));
    hw_ports = (int*) xmalloc(LAG_PORT_LIMIT * sizeof(int));
    ofports = (struct bcmsdk_provider_ofport_node **)
        xmalloc(LAG_PORT_LIMIT * sizeof(struct bcmsdk_provider_ofport_node*));
