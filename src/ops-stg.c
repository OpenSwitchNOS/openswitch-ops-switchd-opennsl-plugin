...
stg_data_find_create(int stgid, bool create)
{
    ...
    p_stg_data = malloc(sizeof(ops_stg_data_t));
    if (!p_stg_data) {
...
int ops_stg_delete(opennsl_stg_t stgid)
{
    ...
    if (NULL == stg_data_find_create(stgid, false)) {
        VLOG_ERR("stg data not found for f=given stgid %d",
                 stgid);
        return -1;
    }
...
/*-----------------------------------------------------------------------------
| Function: ops_stg_list_delete
| Description: delete spanning tree group list
| Parameters[in]: opennsl_stg_t object list
| Parameters[out]: count: number of groups in list
| Return: error value
-----------------------------------------------------------------------------*/
int ops_stg_list_delete(opennsl_stg_t *list_ptr, int count)
{
   //TODO
   return 0;
}
