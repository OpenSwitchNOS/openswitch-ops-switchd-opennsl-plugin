                                   ...
                                   const uint32_t egr_num_id)
{
    ...
    snprintf(egress_object_id_key, 12, "%d", l3_egress_id); 
                                      ...
                                      const int l3_egress_id)
{
    ...
    rc = opennsl_l3_egress_stat_counter_get(0, egress_id_node->egress_object_id,
    ...
    rc = opennsl_l3_egress_stat_counter_get(0, egress_id_node->egress_object_id,
                                        ...
                                        &(count_arr[0]));
    if (rc) {
        VLOG_ERR("Failed to get deleted drop stat counter for l3 egress id: %d",
                 l3_egress_id);
        return 1; /* Return error */
    }
    VLOG_DBG(" deleted drop packets obtained for l3 egress id: %d",
              l3_egress_id);
    netdev->deleted_stats_counter.del_drop_packets += count_arr[0].packets;
                           ...
                           struct netdev_stats *stats)
{
    ...
    HMAP_FOR_EACH(egress_id_node, egress_node, &(netdev->egress_id_map)) {
        VLOG_DBG("Iterating through hmap for id: %d",
                  egress_id_node->egress_object_id);
        rc = bcmsdk_get_l3_egress_stats(del_stats, stats, 
                                        egress_id_node->egress_object_id,
                                        egress_id_node->egress_num_id);
        if (rc) {
...
#if 0
    stats->tx_dropped += del_stats->del_drop_packets; 
#endif

    /* Now get the ingress stats for the l3 interface */
    rc = bcmsdk_get_l3_ingress_stats(del_stats, stats,
