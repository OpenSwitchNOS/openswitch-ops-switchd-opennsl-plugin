...
netdev_bcmsdk_l3intf_fp_stats_destroy(opennsl_port_t hw_port, int hw_unit)
{
    ...
    for(i = 0; i< NUM_L3_FP_STATS; i++)
    {
        ...
        if(fp_entries && fp_entries[i] > 0)
        {
            ...
            rc = ops_destroy_l3_feature_entry_grp(hw_unit, fp_entries[i]);
            if (OPENNSL_FAILURE(rc)) {
                VLOG_ERR("Error while destroying FP stats \
                        Unit=%d portid=%d. rc=%s",
                        hw_unit, hw_port, opennsl_errmsg(rc));
...
{
    opennsl_error_t rc = OPENNSL_E_NONE;
    VLOG_DBG("%s Add entry to group = %d, vlan_id = %d", __FUNCTION__, l3_feature_grpid, vlan_id);
    rc = opennsl_field_entry_create(unit, l3_feature_grpid, entry_id);
    if (OPENNSL_FAILURE(rc)) {
    ...
    rc = opennsl_field_stat_create(unit, l3_feature_grpid, 2, stat_ifp, stat_id);
    if (OPENNSL_FAILURE(rc)) {
...
{
    opennsl_error_t rc = OPENNSL_E_NONE;
    VLOG_DBG("%s Add entry to group = %d, port = %d", __FUNCTION__, l3_feature_grpid, portid);
    rc = opennsl_field_entry_create(unit, l3_feature_grpid, entry_id);
    if (OPENNSL_FAILURE(rc)) {
