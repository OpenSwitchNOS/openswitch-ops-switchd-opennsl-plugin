...
netdev_internal_bcmsdk_update_flags(struct netdev *netdev_,
                                    ...
                                    enum netdev_flags *old_flagsp)
{
    /*  In the simulated switch, we ignore the incoming flags as the underlying
     *  hardware responsible to change the status of the flags is absent.
     *  Thus, we set new flags to preconfigured values.
     *  We follow the same logic here for internal interfaces. */
    struct netdev_bcmsdk *netdev = netdev_bcmsdk_cast(netdev_);
    if ((off | on) & ~NETDEV_UP) {
        return EOPNOTSUPP;
    }
