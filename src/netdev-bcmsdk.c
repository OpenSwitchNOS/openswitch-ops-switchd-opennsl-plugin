...
struct netdev_bcmsdk {
    ...
    /* This flag indicates whether iptables rules for statistics are installed.
    * Used to skip fetching stats from iptables for L2 interfaces */
    bool xtable_stat_rules_installed;
};
...
netdev_bcmsdk_construct(struct netdev *netdev_)
{
    ...
    netdev->xtable_stat_rules_installed = false;
...
static int
netdev_bcmsdk_get_kernel_stats_util(const char *if_name,
                                    struct kernel_l3_tx_stats *kernel_stats,
                                    const char *xtables_cmd)
{
    ...
    fp = popen(cmd, "r");
    if (fp == NULL) {
        VLOG_ERR("Failed to get %s TX stats from kernel for l3 if %s", xtables_cmd, if_name);
        return 1;
    }
    ...
    if(fgets(path, sizeof(path)-1, fp))
    {
        ...
        {
            VLOG_DBG("Failed to parse %s TX ucast stats for l3 if %s", xtables_cmd, if_name);
            pclose(fp);
            return 1;
        }
        if(!strcmp(xtables_cmd, "iptables"))
        {
            kernel_stats->ipv4_uc_tx_packets += packets;
            kernel_stats->ipv4_uc_tx_bytes += bytes;
        }
        else
        {
            kernel_stats->ipv6_uc_tx_packets += packets;
            kernel_stats->ipv6_uc_tx_bytes += bytes;
        }
    }
    packets = 0;
    bytes = 0;
    ...
    if(fgets(path, sizeof(path)-1, fp))
    {
        if(sscanf(path, "%"PRIu64"%"PRIu64, &packets, &bytes) < 2)
        {
            VLOG_DBG("Failed to parse %s TX mcast stats for l3 if %s", xtables_cmd, if_name);
            pclose(fp);
            return 1;
        }
        if(!strcmp(xtables_cmd, "iptables"))
        {
            kernel_stats->ipv4_mc_tx_packets += packets;
            kernel_stats->ipv4_mc_tx_bytes += bytes;
        }
        else
        {
            kernel_stats->ipv6_mc_tx_packets += packets;
            kernel_stats->ipv6_mc_tx_bytes += bytes;
        }
    }
...
static int
netdev_bcmsdk_get_kernel_egress_stats(const char *if_name, struct netdev_stats *stats)
{
    int rc = 0;
    struct kernel_l3_tx_stats kernel_stats;

    memset(&kernel_stats, 0, sizeof(kernel_stats));
    rc = netdev_bcmsdk_get_kernel_stats_util(if_name, &kernel_stats, "iptables");
    if(rc){
        return rc;
    }
    rc = netdev_bcmsdk_get_kernel_stats_util(if_name, &kernel_stats, "ip6tables");
    if(rc){
        return rc;
    }

    stats->l3_uc_tx_packets += kernel_stats.ipv4_uc_tx_packets + kernel_stats.ipv6_uc_tx_packets;
    stats->l3_mc_tx_packets += kernel_stats.ipv4_mc_tx_packets + kernel_stats.ipv6_mc_tx_packets;

    stats->l3_uc_tx_bytes += kernel_stats.ipv4_uc_tx_bytes + kernel_stats.ipv6_uc_tx_bytes;
    stats->l3_mc_tx_bytes += kernel_stats.ipv4_mc_tx_bytes + kernel_stats.ipv6_mc_tx_bytes;

    stats->ipv4_uc_tx_packets += kernel_stats.ipv4_uc_tx_packets;
    stats->ipv4_mc_tx_packets += kernel_stats.ipv4_mc_tx_packets;
    stats->ipv6_uc_tx_packets += kernel_stats.ipv6_uc_tx_packets;
    stats->ipv6_mc_tx_packets += kernel_stats.ipv6_mc_tx_packets;

    stats->ipv4_uc_tx_bytes += kernel_stats.ipv4_uc_tx_bytes;
    stats->ipv4_mc_tx_bytes += kernel_stats.ipv4_mc_tx_bytes;
    stats->ipv6_uc_tx_bytes += kernel_stats.ipv6_uc_tx_bytes;
    stats->ipv6_mc_tx_bytes += kernel_stats.ipv6_mc_tx_bytes;
                           ...
                           struct netdev_stats *stats)
{
    ...
    /* Fetch Kernel egress stats if they were installed */
    if(netdev->xtable_stat_rules_installed)
    {
        rc = netdev_bcmsdk_get_kernel_egress_stats(netdev->up.name, stats);
    }
...
/* Utility function to check if a specific iptables rule is installed*/
static bool
netdev_bcmsdk_l3stats_xtables_rule_check(const char *if_name, const char *xtables_cmd,
                                         const char *pkttype)
{
    char cmd[256];
    int rc = 0;

    snprintf(cmd, sizeof(cmd), "%s %s -C OUTPUT -o %s -m pkttype --pkt-type %s",
            SWNS_EXEC, xtables_cmd, if_name, pkttype);
    rc = system(cmd);
    if (rc != 0) {
        return false;
    }
...
/* Utility function to delete one specific iptables rule */
static int
netdev_bcmsdk_l3stats_xtables_rule_delete(const char *if_name, const char *xtables_cmd,
                                          const char *pkttype)
{
    char cmd[256];
    int rc = 0;

    /* Try to delete rule only if it exists */
    if(netdev_bcmsdk_l3stats_xtables_rule_check(if_name, xtables_cmd, pkttype))
    {
        snprintf(cmd, sizeof(cmd), "%s %s -D OUTPUT -o %s -m pkttype --pkt-type %s",
                SWNS_EXEC, xtables_cmd, if_name, pkttype);
        rc = system(cmd);
        if (rc != 0) {
            VLOG_ERR("Failed to exec command - %s (rc=%d)", cmd, rc);
            return 1;
        }
    }
    return 0;
...
/* Utility function to install one specific iptables rule */
static int
netdev_bcmsdk_l3stats_xtables_rule_add(const char *if_name, const char *xtables_cmd,
                                       const char *pkttype)
{
    char cmd[256];
    int rc = 0;
    ...
    if(!netdev_bcmsdk_l3stats_xtables_rule_check(if_name, xtables_cmd, pkttype))
    {
        snprintf(cmd, sizeof(cmd), "%s %s -A OUTPUT -o %s -m pkttype --pkt-type %s",
                SWNS_EXEC, xtables_cmd, if_name, pkttype);
        rc = system(cmd);
        if (rc != 0) {
            VLOG_ERR("Failed to exec command - %s (rc=%d)", cmd, rc);
...
/* Install xtables rules to account for slow path L3 egress stats */
int
netdev_bcmsdk_l3stats_xtables_rules_create(struct netdev *netdev_)
{
    struct netdev_bcmsdk *netdev = netdev_bcmsdk_cast(netdev_);
    int rc = 0;

    rc = netdev_bcmsdk_l3stats_xtables_rule_add(netdev_->name, "iptables", "unicast");
    rc = netdev_bcmsdk_l3stats_xtables_rule_add(netdev_->name, "iptables", "multicast");
    rc = netdev_bcmsdk_l3stats_xtables_rule_add(netdev_->name, "ip6tables", "unicast");
    rc = netdev_bcmsdk_l3stats_xtables_rule_add(netdev_->name, "ip6tables", "multicast");

    netdev->xtable_stat_rules_installed = true;

    return rc;
}

/* Delete xtables rules created for slow path L3 egress stats */
int
netdev_bcmsdk_l3stats_xtables_rules_delete(struct netdev *netdev_)
{
    struct netdev_bcmsdk *netdev = netdev_bcmsdk_cast(netdev_);
    int rc = 0;

    rc = netdev_bcmsdk_l3stats_xtables_rule_delete(netdev_->name, "iptables", "unicast");
    rc = netdev_bcmsdk_l3stats_xtables_rule_delete(netdev_->name, "iptables", "multicast");
    rc = netdev_bcmsdk_l3stats_xtables_rule_delete(netdev_->name, "ip6tables", "unicast");
    rc = netdev_bcmsdk_l3stats_xtables_rule_delete(netdev_->name, "ip6tables", "multicast");
...
netdev_bcmsdk_l3_global_stats_destroy(struct netdev *netdev_)
{
    ...
    netdev_bcmsdk_l3stats_xtables_rules_delete(netdev_);
    return OPENNSL_E_NONE;
}
