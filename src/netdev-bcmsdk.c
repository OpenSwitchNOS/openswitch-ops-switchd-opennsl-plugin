                                    ...
                                    bool is_v6)
{
    ...
    snprintf(cmd, sizeof(cmd), "%s %s -S OUTPUT -v | awk '$4 == \"%s\" {print $10,$11}'",
             SWNS_EXEC, is_v6 ? "ip6tables" : "iptables", if_name);

    /* Open the command for reading. */
    fp = popen(cmd, "r");
    if (fp == NULL) {
        VLOG_ERR("Failed to open pipe for command %s", cmd);
        return 1;
    }
    ...
    if(fgets(path, sizeof(path)-1, fp))
    {
        ...
        {
            VLOG_DBG("Failed to parse TX ucast stats for L3 interface %s", if_name);
            pclose(fp);
            return 1;
        }
        if(is_v6)
    ...
    if(fgets(path, sizeof(path)-1, fp))
    {
        if(sscanf(path, "%"PRIu64"%"PRIu64, &packets, &bytes) < 2)
        {
            VLOG_DBG("Failed to parse TX mcast stats for L3 interface %s", if_name);
            pclose(fp);
            return 1;
        }
        if(is_v6)
...
netdev_bcmsdk_get_kernel_egress_stats(const char *if_name, struct netdev_stats *stats)
{
    ...
    rc = netdev_bcmsdk_get_kernel_stats_util(if_name, &kernel_stats, false);
    if(rc){
        return rc;
    }
    rc = netdev_bcmsdk_get_kernel_stats_util(if_name, &kernel_stats, true);
    if(rc){
        return rc;
    }
