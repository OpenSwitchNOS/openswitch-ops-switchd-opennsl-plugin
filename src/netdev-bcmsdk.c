...
int
netdev_bcmsdk_get_subint_ref_count(struct netdev *netdev_)
{
    struct netdev_bcmsdk *netdev = netdev_bcmsdk_cast(netdev_);
    struct netdev *parent = NULL;
    struct netdev_bcmsdk *parent_netdev = NULL;
    ...
    if (parent != NULL) {
        parent_netdev = netdev_bcmsdk_cast(parent);
        ref_count = parent_netdev->subint_ref_count;
        netdev_close(parent);
    }
    return ref_count;
}
...
void
netdev_bcmsdk_update_subint_ref_count(struct netdev *netdev_, bool increment)
{
    struct netdev_bcmsdk *netdev = netdev_bcmsdk_cast(netdev_);
    struct netdev *parent = NULL;
    struct netdev_bcmsdk *parent_netdev = NULL;
                                    ...
                                    enum netdev_flags *old_flagsp)
{
    ...
    if (netdev->parent_netdev_name != NULL) {
        ...
        if (parent != NULL) {
            ...
            VLOG_INFO("%s Set state for name=%s unit=%d port=%d",
                    __FUNCTION__,
