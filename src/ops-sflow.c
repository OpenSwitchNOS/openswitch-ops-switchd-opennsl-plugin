...
int get_ether_type (uint8 *byte)
{
    if (byte == NULL) {
        return 0;
    }

    if (byte[0] == 0x08 && byte[1] == 0x00) {
        return IPV4;
    }

    if (byte[0] == 0x08 && byte[1] == 0x06) {
        return ARP;
    }

    if (byte[0] == 0x81 && byte[1] == 0x00) {
        return VLAN;
    }

    if (byte[0] == 0x80 && byte[1] == 0x35) {
        return RARP;
    }

    if (byte[0] == 0x88 && byte[1] == 0xCC) {
        return LLDP;
    }
...
print_pkt(const opennsl_pkt_t *pkt)
{
    ...
    for(i=0; i<pkt->blk_count; i++) {
        ...
        e = get_ether_type(&pkt->pkt_data[i].data[12]);
...
void ops_sflow_write_sampled_pkt(opennsl_pkt_t *pkt)
{
    ...
    if ((get_ether_type(&pkt->pkt_data[0].data[12]) == VLAN) &&
