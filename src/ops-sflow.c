              ...
              void *aux OVS_UNUSED)
{
    ...
    if (port) { /* set for specific port */
        ...
        if (ops_sflow_agent) {
            /* TODO: How does input port (1, 2, 3 etc) translate to ifIndex?
             * 1-->1, 2-->2 and so on? Can a switch have non-numerical port
             * numbers (such as 10/1, 20/4 etc)? */
            ifIndex = if_nametoindex(argv[1]);
            sampler = ops_sflow_agent->jumpTable[ifIndex];

            /* There is no sampler for ifIndex */
            if (sampler == NULL) {
        ...
        OPENNSL_PBMP_ITER (port_config.e, tempPort) {
            ...
            /* TODO: is ifIndex same as tempPort? Perhaps on as5712. */
            VLOG_ERR("tempPort=%d", tempPort);
            sampler = ops_sflow_agent->jumpTable[tempPort];

            /* There is no sampler for ifIndex */
            if (sampler == NULL) {
                VLOG_ERR("[%s:%d]: There is no Sampler for port: %d", __FUNCTION__, __LINE__, tempPort);
                goto done;
            }
            /* TODO: ingress rate or egress rate? Pick ingress, for now. */
            sfl_sampler_set_sFlowFsPacketSamplingRate(sampler, ingress_rate);
...
ops_sflow_enable_agent ()
{
    ...
    /* RECEIVER: configure a receiver (aka Collector) */
    ...
    /* TODO: In OPS, default rate is 4096. In OvS, its 400. Resolve! */
    ...
    /* POLLER:
     * TODO: configure a poller. Configure 'magic' and counter function
     * (currently set to NULL) */
    poller = sfl_agent_addPoller(ops_sflow_agent, &dsi, NULL, NULL);
    sfl_poller_set_sFlowCpInterval(poller, 20);
    sfl_poller_set_sFlowCpReceiver(poller, 1);
