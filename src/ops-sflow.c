...
/* Given a (fron panel) port number, verify if that port is prohibted for
 * sFlow sampling. true - disable sFlow; false - enable sFlow
 */
static bool
disable_sflow_port(opennsl_port_t   tempPort)
{
    ...
    SSET_FOR_EACH(port_name, &sflow_options->ports) {
        ...
        if (hw_port == tempPort) {
            VLOG_ERR("sFlow disabled on port: %d", hw_port);
            return true;
        }
    }
                            ...
                            const int ingress_rate, const int egress_rate)
{
    ...
    if (port) { /* set for specific port */
        ...
        OPENNSL_PBMP_ITER (port_config.e, tempPort) {
            ...
            if (disable_sflow_port(tempPort)) {
                continue;
            }
        ...
        /* TODO: Clear rate on sFlow disabled ports */
