...
bundle_destroy(struct ofbundle *bundle)
{
    ...
    VLOG_DBG("LAG : bundle destroy, destroying bundle = %s\n", bundle->name);

    LIST_FOR_EACH_SAFE (port, next_port, bundle_node, &bundle->ports) {

        VLOG_DBG("LAG : port_t->up.ofp_port = %d\n", port->up.ofp_port);
        ...
        if (strcmp(type, OVSREC_INTERFACE_TYPE_SYSTEM) == 0) {
            ...
            if (bundle->l3_intf) {
                ...
                VLOG_DBG("LAG : destroy the l3 interface hw_port = %d\n", hw_port);
    ...
    if (bundle->bond_hw_handle != -1) {
        VLOG_DBG("LAG : destroy the lag %s\n", bundle->name);
           ...
           const struct ofproto_bundle_settings *s)
{
    ...
    VLOG_DBG("LAG : %s: entry, ofproto_=%p, aux=%p, s=%p",
    ...
    if (s == NULL) {
        if (bundle != NULL) {
            VLOG_DBG("LAG : deleting bundle %s", bundle->name);
    ...
    if ((-1 == bundle->bond_hw_handle) &&
        ...
        VLOG_DBG("LAG : %s: Allocated bond_hw_handle# %d for port %s",
    ...
    if (ofproto->vrf &&
        ...
        VLOG_DBG("LAG : s-enable = %d vlan_id(%d)\n",s->enable, vlan_id);

        if (bundle->l3_intf) {
            VLOG_DBG("LAG : bundle %s exists\n", bundle->name);
            ...
            if ((bundle->l3_intf->l3a_vid != vlan_id || !s->enable) &&
                ...
                if (strcmp(type, OVSREC_INTERFACE_TYPE_VLANSUBINT) == 0) {
                    ...
                    VLOG_DBG("LAG : disable l3 interface %s\n", bundle->name);
        ...
        if (vlan_id && !bundle->l3_intf && s->enable) {
            ...
            if (strcmp(type, OVSREC_INTERFACE_TYPE_SYSTEM) == 0) {
                VLOG_DBG("LAG : Create interface %s\n", bundle->name);
        ...
        if (ofproto->vrf &&
           ...
           (bundle->l3_intf && s->hw_bond_should_exist)) {
            ...
            for (i = 0; i < s->n_slaves; i++) {
                ...
                VLOG_DBG("LAG : Working on port %s for add", netdev_get_name(port->up.netdev));
                netdev_bcmsdk_get_hw_info(port->up.netdev, &hw_unit, &hw_port, mac);
            ...
            LIST_FOR_EACH_SAFE (port, next_port, bundle_node, &bundle->ports) {
                ...
                OPENNSL_PBMP_CLEAR(lag_pbmp);
                netdev_bcmsdk_get_hw_info(port->up.netdev, &hw_unit, &hw_port, mac);
                VLOG_DBG("LAG : Working on port %s for delete", netdev_get_name(port->up.netdev));
                OPENNSL_PBMP_PORT_ADD(lag_pbmp, hw_port);
                if (s->n_slaves == 0) {
                    ops_routing_disable_l3_interface(hw_unit, hw_port,
                            bundle->l3_intf,
    ...
    VLOG_DBG("LAG : 1. list_size(&bundle->ports) = %zu, s->n_slaves %zu\n",
    ...
    if (!ok || list_size(&bundle->ports) != s->n_slaves) {
        ...
        LIST_FOR_EACH_SAFE (port, next_port, bundle_node, &bundle->ports) {
            ...
            VLOG_DBG("LAG : Bundle delete port %s\n", netdev_get_name(port->up.netdev));
            bundle_del_port(port);
        found: ;
        }
    }
    VLOG_DBG("LAG : 2. list_size(&bundle->ports) = %zu, s->n_slaves %zu\n",
               list_size(&bundle->ports), s->n_slaves);

    ovs_assert(list_size(&bundle->ports) <= s->n_slaves);

    VLOG_DBG("LAG : 3. list_size(&bundle->ports) = %zu, s->n_slaves %zu\n",
               list_size(&bundle->ports), s->n_slaves);

    if (list_is_empty(&bundle->ports)) {
        if (s->hw_bond_should_exist == false) {
            VLOG_DBG("LAG : calling bundle destroy\n");
            bundle_destroy(bundle);
            return 0;
        }
    }
...
bundle_remove(struct ofport *port_ OVS_UNUSED)
{
    ...
    if (bundle) {
        if (bundle->bond_hw_handle == -1) {
        VLOG_DBG("LAG : bundle remove, removing bundle %s\n", bundle->name);
            bundle_del_port(port);
            if (list_is_empty(&bundle->ports)) {
                bundle_destroy(bundle);
            }
