    ...
    switch (act->type) {
        case OFPACT_OUTPUT: {
            ...
            switch (port_no) {
                case OFPP_CONTROLLER: {
                    switch(flow->tableId) {
                        case OFDPA_FLOW_TABLE_ID_BRIDGING:
                            ...
                            break;
                        default:
                            VLOG_ERR("Upsupported output port action (OFPP_CONTROLLER) for Table: %d", flow->tableId);
                ...
                case OFPP_IN_PORT:
                case OFPP_NORMAL:
                    VLOG_ERR("[%s %d]Unsupported output port 0x%x", __FUNCTION__, __LINE__, port_no);
                ...
                default:
                    ...
                    VLOG_ERR("[%s:%d] Unsupported output port %d for Table %d",
                                __FUNCTION__,__LINE__,port_no, flow->tableId);
                    return OFPERR_OFPBIC_UNSUP_INST;
                    break;
                }
            break;
        ...
        case OFPACT_GROUP: {
            ...
            switch(flow->tableId) {
                ...
                default:
                    VLOG_ERR("[%d]Unsupported action: '%s' (%d), table %d", __LINE__, ofpact_name(act->type), act->type, flow->tableId);
                    return OFPERR_OFPHFC_INCOMPATIBLE;
            }
            break;
        }
            ...
            case OVSINST_OFPIT11_WRITE_ACTIONS:
                switch(flow->tableId) {
                    ...
                    case OFDPA_FLOW_TABLE_ID_BRIDGING:
                        break;
                    default:
                        VLOG_ERR("[%d] Unsupported instruction table %d", __LINE__, flow->tableId);
                        return OFPERR_OFPHFC_INCOMPATIBLE;
                }
          ...
          case OFPACT_OUTPUT:
              {
                  ...
                  switch (port_no) {
                    ...
                    case OFPP_IN_PORT:
                    case OFPP_NORMAL:
                        VLOG_ERR("[%s %d] unsupported output port 0x%x",
...
port_construct(struct ofport *port_)
{
    ...
    if (ofproto_is_ofdpa(port->up.ofproto)) {
        ...
        else
        {
            ...
            if (rc != OFDPA_E_NONE) {
                VLOG_DBG("Error creating port in OFDPA. (rc = %d, ofp_port = %d, hw_unit = %d, hw_port = %d)\n",
...
port_destruct(struct ofport *port_)
{
    ...
    if (ofproto_is_ofdpa(port->up.ofproto)) {
        ...
        if (rc != OFDPA_E_NONE) {
            VLOG_DBG("Error deleting port from OFDPA. (rc = %d, ofp_port = %d)\n",
                     rc, port->up.ofp_port);
        }
    }
           ...
           const struct ofproto_bundle_settings *s)
{
    ...
    /* OF-DPA currently only supports physical ports */
...
OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (ofproto_is_ofdpa(rule->up.ofproto)) {
        ...
        if(error) {
            VLOG_ERR("[ERROR = %d] in ofdpa %s, %d", error, __FUNCTION__,__LINE__);
        }
        ovs_mutex_unlock(&rule->stats_mutex);
    }
