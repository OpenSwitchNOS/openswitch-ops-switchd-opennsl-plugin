        ...
        int *hw_unit, int *hw_port)
{
    ...
    if ((bundle->hw_unit != -1) && (bundle->hw_port != -1)) {
        DEBUG_MIRROR("using cached values hw_unit %d hw_port %d for port %s",
    ...
    if (list_size(&bundle->ports) == 1) {
        ...
        ERROR_MIRROR("port list size is %d for bundle %s",
            (int) list_size(&bundle->ports), bundle->name);
        *hw_unit = *hw_port = -1;
    }

    DEBUG_MIRROR("bundle %s (0x%p): obtained hw_unit %d hw_port %d",
    ...
    void *aux, const struct ofproto_mirror_settings *s)
{
    ...
    DEBUG_MIRROR("mirror_set__ called");
