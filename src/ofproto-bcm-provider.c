...
static void tnl_key_configure(int action, struct ofbundle *bundle, const struct ofproto_bundle_settings *s, opennsl_pbmp_t *temp_pbm);
...
static int
bundle_get_tnl_key(const struct ofproto_bundle_settings *s)
{
    int i;
    if(!s) {
    ...
    for (i=0; i < s->binding_cnt; i++) {
        if(s->vlan == s->binding_vlans[i] && s->vlan_mode == PORT_VLAN_ACCESS)
        {
            return s->binding_tunnel_keys[i];
        }
    }
    return -1;
}
...
static void
tnl_key_configure(int action, struct ofbundle *bundle, const struct ofproto_bundle_settings *s, opennsl_pbmp_t *temp_pbm)
{
    if(action == TUNNEL_KEY_UNBIND) {
        if(bundle->tunnel_key != -1 &&
           !ops_vport_unbind_access_port(bundle->hw_unit, *temp_pbm, bundle->tunnel_key)) {
            bundle->tunnel_key = -1;
        ...
        if((new_vni != -1) &&
            !ops_vport_bind_access_port(bundle->hw_unit, *temp_pbm, new_vni, s->vlan)) {
 ...
 * Return the prefix string x.x.x.x/y
 */
static char*
find_prefix_from_port ( int hw_port, bool is_ipv6)
{
    struct ofbundle *bundle;
    struct bcmsdk_provider_node *ofproto;
    HMAP_FOR_EACH (ofproto, all_bcmsdk_provider_node, &all_bcmsdk_provider_nodes) {
        if(ofproto->vrf) {
...
bool
ofproto_find_ip4_from_port(int hw_port, uint32_t *ip)
{
        ...
        struct ofproto_route *routep)
{
    switch (action)
    {
        ...
        case OFPROTO_ROUTE_DELETE:
            ...
            break;
        default:
            VLOG_DBG("******* TODO OFPROTO_ROUTE_DELETE_NH ********");
            break;
    }
}

static void
vport_update_host( struct ofbundle *bundle, int event, bool is_ipv6_addr, char *ip_addr,
        char *next_hop_mac_addr, int *l3_egress_id)
{
    VLOG_DBG("%s  on port %d ip address %s, l3_egress_id %d "
              ...
              next_hop_mac_addr);
    if(!is_ipv6_addr) {
        netdev_vport_update_host_chg(event, bundle->hw_port, ip_addr, *l3_egress_id);
    }
}
                   ...
                   struct ofproto_logical_switch *log_switch)
{
    ...
    switch (action) {
    case OFPROTO_LOG_SWITCH_ACTION_ADD:
        opcode = BCMSDK_VXLAN_OPCODE_CREATE;

        lsw.vnid = log_switch->tunnel_key;

        break;
    case OFPROTO_LOG_SWITCH_ACTION_DEL:
        lsw.vnid = log_switch->tunnel_key;
        opcode = BCMSDK_VXLAN_OPCODE_DESTROY;
        break;
    case OFPROTO_LOG_SWITCH_ACTION_MOD:
    default:
