...
bundle_destroy(struct ofbundle *bundle)
{
    ...
    VLOG_DBG("%s, destroying bundle = %s", __func__, bundle->name);
        ...
        if (strcmp(type, OVSREC_INTERFACE_TYPE_SYSTEM) == 0) {
            ...
            if (bundle->l3_intf) {
                ...
                if (list_size(&bundle->ports) > 1) {
                    ...
                    VLOG_DBG("bundle_destroy Working on port %s for delete",
                    ...
                    netdev_bcmsdk_get_hw_info(port->up.netdev, &hw_unit, &hw_port, mac);
                    VLOG_DBG("destroy the l3 interface hw_port = %d", hw_port);
            ...
            VLOG_DBG("destroy the subinterface %s", bundle->name);
            if (bundle->l3_intf) {
           ...
           const struct ofproto_bundle_settings *s)
{
    ...
    if (ofproto->vrf &&
        ...
        VLOG_DBG("s-enable = %d vlan_id(%d)\n",s->enable, vlan_id);

        if (bundle->l3_intf) {
            VLOG_DBG("bundle %s exists\n", bundle->name);
            ...
            if ((bundle->l3_intf->l3a_vid != vlan_id || !s->enable) &&
                ...
                if (strcmp(type, OVSREC_INTERFACE_TYPE_VLANSUBINT) == 0) {
                    ...
                    VLOG_DBG("disable l3 interface %s\n", bundle->name);
        ...
        if (ofproto->vrf &&
           ...
           (bundle->l3_intf && s->hw_bond_should_exist)) {
            ...
            for (i = 0; i < s->n_slaves; i++) {
                ...
                VLOG_DBG("Adding port %s from VLAN", netdev_get_name(port->up.netdev));
                netdev_bcmsdk_get_hw_info(port->up.netdev, &hw_unit, &hw_port, mac);
                OPENNSL_PBMP_PORT_ADD(lag_pbmp, hw_port);
                /* add bitmap */
            ...
            LIST_FOR_EACH_SAFE (port, next_port, bundle_node, &bundle->ports) {
                ...
                OPENNSL_PBMP_CLEAR(lag_pbmp);
                netdev_bcmsdk_get_hw_info(port->up.netdev, &hw_unit, &hw_port, mac);
                VLOG_DBG("Removing port %s from VLAN", netdev_get_name(port->up.netdev));
                OPENNSL_PBMP_PORT_ADD(lag_pbmp, hw_port);
                if (s->n_slaves == 0) {
                    ops_routing_disable_l3_interface(hw_unit, hw_port,
                            bundle->l3_intf,
