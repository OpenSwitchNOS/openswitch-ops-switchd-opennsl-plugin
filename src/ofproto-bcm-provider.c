...
static int
port_l3_host_add(struct ofproto *ofproto, bool is_ipv6, char *ip_addr)
{
    ...
    /* Call Provider */
    if (!ofproto->ofproto_class->l3_host_action(ofproto, OFPROTO_HOST_ADD,
                                                &host_info)) {
        VLOG_DBG("Added host entry for %s", ip_addr);
        return 0;
    } else {
        VLOG_ERR("l3_host_action for add failed");
        return 1;
    }
} /* port_l3_host_add */
...
static int
port_l3_host_delete(struct ofproto *ofproto, bool is_ipv6, char *ip_addr)
{
    struct ofproto_l3_host host_info;
    ...
    /* Call Provider */
    if (!ofproto->ofproto_class->l3_host_action(ofproto, OFPROTO_HOST_DELETE,
                                                &host_info)) {
        VLOG_DBG("Deleted host entry for %s", ip_addr);
        return 0;
    } else {
        VLOG_ERR("l3_host_action for delete failed");
        return 1;
    }
} /* port_l3_host_delete */
                    ...
                    const struct ofproto_bundle_settings *s)
{
    ...
    if (ofproto->ofproto_class->l3_host_action == NULL) {
       VLOG_DBG("No ofproto registered for l3 ip/host reconfig");
       return 0;
    }
