...
bundle_destroy(struct ofbundle *bundle)
{
    ...
    VLOG_DBG("Bundle_destroy\n");
    if (!bundle) {
        return;
    }

    port = get_ofp_port(bundle->ofproto, bundle->slaves[0]);
    ...
    if (strcmp(type, OVSREC_INTERFACE_TYPE_SYSTEM) != 0) {
        ...
        ops_routing_disable_l3_subinterface(bundle->hw_unit, bundle->hw_port, bundle->l3_intf, port->up.netdev);
           ...
           const struct ofproto_bundle_settings *s)
{
    ...
    VLOG_INFO("%s: entry, ofproto_=%p, aux=%p, s=%p",
    ...
    if (list_is_empty(&bundle->ports)) {
        VLOG_DBG("2 deleting bundle %s", bundle->name);
        bundle_destroy(bundle);
        return 0;
    }

    VLOG_DBG("Copying s->n_slaves to bundle: %zu\n", s->n_slaves);
    bundle->n_slaves = s->n_slaves;
    bundle->slaves = xmalloc(list_size(&bundle->ports) * sizeof *bundle->slaves);
    for(i = 0; i < s->n_slaves; i++) {
        bundle->slaves[i] = s->slaves[i];
    }
    ...
    if (ofproto->vrf &&
        ...
        VLOG_DBG("get the port for slave\n");
        ...
        if (bundle->l3_intf) {
            ...
            if (bundle->l3_intf->l3a_vid != vlan_id || !s->enable) {
                VLOG_DBG("call disble because s-enable = %d or bundle->l3_intf->l3a_vid(%d) != vlan_id(%d)\n",
                           s->enable, bundle->l3_intf->l3a_vid, vlan_id);
                if (strcmp(type, OVSREC_INTERFACE_TYPE_VLANSUBINT) == 0) {
                    VLOG_DBG("disable l3 subinterface");
                    ops_routing_disable_l3_subinterface(hw_unit, hw_port, bundle->l3_intf, port->up.netdev);
                } else if (strcmp(type, OVSREC_INTERFACE_TYPE_SYSTEM) == 0) {
                    ops_routing_disable_l3_interface(hw_unit, hw_port, bundle->l3_intf, port->up.netdev);
                } else if (strcmp(type, OVSREC_INTERFACE_TYPE_INTERNAL) == 0) {
    ...
    if (s->vlan_mode != PORT_VLAN_ACCESS) {
        VLOG_DBG("It is not ACCESS mode, mode = %d\n", s->vlan_mode);
        if (s->trunks != NULL) {
            new_trunks = s->trunks;
        } else {
            new_trunks = ofproto_->vlans_bmp;
            trunk_all_vlans = true;
            VLOG_DBG("trunk_all_vlans = %d\n", trunk_all_vlans);
        }
    }
...
bundle_remove(struct ofport *port_ OVS_UNUSED)
{
    ...
    VLOG_DBG("Bundle remove ()\n");
    if (bundle) {
        bundle_del_port(port);
        if (list_is_empty(&bundle->ports)) {
            VLOG_DBG("Calling bundle destroy\n");
            bundle_destroy(bundle);
        }
    }
}
