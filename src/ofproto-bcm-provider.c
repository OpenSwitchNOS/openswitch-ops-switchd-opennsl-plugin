         ...
         const struct ofproto_sflow_options *oso)
{
    ...
    if (strcmp(sflow_options->agent_device, oso->agent_device) != 0) {
        sflow_options->agent_device = oso->agent_device;

        __af = strchr(oso->agent_device, ':');
        if (__af == NULL) { /* IPv4 */
            ops_sflow_agent_ip(oso->agent_device, AF_INET);
        } else {
            ops_sflow_agent_ip(oso->agent_device, AF_INET6);
        }
    }
