    ...
    if (bundle) {
        ...
        LIST_FOR_EACH_SAFE (port, next_port, bundle_node, &bundle->ports) {
            ...
            if (VALID_HW_UNIT(hw_unit) && VALID_HW_UNIT_PORT(hw_unit,
                ...
                /*
                 * OPS_TODO:
                 *    Currently PI invokes this API for bridge_normal
                 *    interface also for which this hw unit & port will
                 *    be invalid. For now, return SUCCESS. Otherwise PI will
                 *    invoke the same API every 2 seconds. Once this is fixed
                 *    in PI, return error code.
                 */
