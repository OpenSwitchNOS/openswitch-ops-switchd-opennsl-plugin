...
ops_egress_id_lookup(char*hashstr_l)
{
                    ...
                    void   *userdata)
{
   ...
   if (!(l2addr->flags & OPENNSL_L2_MOVE_PORT)) {
       ...
       VLOG_INFO("ADD call from ASIC, not involving mac-move, for "
                 "unit=%d, vlan=%d, mac=" ETH_ADDR_FMT, unit, l2addr->vid,
                 ETH_ADDR_ARGS(l2addr->mac));
       return;
   }

   VLOG_INFO("ADD call from ASIC (mac-move)");
   ...
   egress_id_node = ops_egress_id_lookup(hashstr);
   if (egress_id_node == NULL) {
       VLOG_INFO("Egress object id NOT found in process cache, possibly "
                 ...
                 unit, hashstr, l2addr->vid, ETH_ADDR_ARGS(l2addr->mac));

       /* Unexpected condition. This shouldn't happen. */
       return;
   }

   VLOG_INFO("egress_id=%d, got from local hashmap. ", egress_id_node->egress_object_id);

   /* Using egress id get egress object from ASIC */
   rc = opennsl_l3_egress_get(unit, egress_id_node->egress_object_id, &egress_object);

   if (rc != OPENNSL_E_NONE) {
       VLOG_INFO("Egress object not found in ASIC for given vlan/mac. rc=%d "
   ...
   VLOG_INFO("Egress ID removed locally due to ADD call from ASIC (mac-move)");
}
                       ...
                       void   *userdata)
{
   ...
   if (!(l2addr->flags & OPENNSL_L2_MOVE_PORT)) {
       /* Only handle _ADD_ due to mac-move */
       VLOG_INFO("DELETE call from ASIC, not involving mac-move, for "
                 "unit=%d, vlan=%d, mac=" ETH_ADDR_FMT, unit, l2addr->vid,
                 ETH_ADDR_ARGS(l2addr->mac));
   ...
   VLOG_INFO("DELETE call from ASIC (mac-move)");
   ...
   VLOG_INFO("Input: mac=" ETH_ADDR_FMT ", vlan=%d, port=%d, l3_intf=%d; "
             "Output: egress_id=%d, rc=%d, got from ASIC. ", ETH_ADDR_ARGS(egress_object.mac_addr),
             egress_object.vlan, egress_object.port, egress_object.intf, egress_object_id, rc);
   ...
   hmap_insert(&ops_mac_move_egress_id_map, &egress_node->node, hash_string(hashstr, 0));

   VLOG_INFO("Egress ID insert/update due to DELETE call from ASIC (mac-move)");
}
