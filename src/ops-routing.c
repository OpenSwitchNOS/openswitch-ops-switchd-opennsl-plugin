                ...
                opennsl_vlan_t vid)
{
   ...
   rc = opennsl_l2_addr_get(hw_unit, mac, vid, &l2addr);
                                     ...
                                     unsigned char *mac)
{
    ...
    if (rc != OPENNSL_E_NONE) {
        VLOG_ERR("l2_addr_add failed: unit=%d port=-1 vlan=%d vrf=%d rc=%s",
                 hw_unit, vlan_id, vrf_id, opennsl_errmsg(rc));
        ...
        VLOG_DBG("l2_addr_add passed: unit=%d port=-1 vlan=%d vrf=%d rc=%s",
                 hw_unit, vlan_id, vrf_id, opennsl_errmsg(rc));
    }
