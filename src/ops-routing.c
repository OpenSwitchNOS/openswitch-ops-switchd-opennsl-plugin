...
static int
l3ecmp_egress_dynamic_mode_set_reset(int unit, opennsl_l3_egress_ecmp_t *ecmp,
                                 ...
                                 opennsl_if_t *ecmp_intfp, bool update)
{
    ...
    if (update){
        ...
        if (ecmp_resilient_flag){
            /* setting the resilient feature required by the ASIC */
            ecmp_grp.dynamic_mode = OPENNSL_L3_ECMP_DYNAMIC_MODE_RESILIENT;
            ecmp_grp.dynamic_size = ECMP_DYN_SIZE_512;
	}
...
ops_routing_ecmp_hash_set(int hw_unit, unsigned int hash, bool enable)
{
    ...
    if ((hash & OFPROTO_ECMP_HASH_RESILIENT) && enable) {
        ecmp_resilient_flag = true;
        rc = opennsl_l3_egress_ecmp_traverse(0, l3ecmp_egress_dynamic_mode_set_reset, NULL);
        if (OPENNSL_FAILURE(rc)) {
            VLOG_ERR("Failed to traverse ECMP groups to Set resilient mode ");
        }
    } else if ((hash & OFPROTO_ECMP_HASH_RESILIENT) && !enable) {
        ecmp_resilient_flag = false;
        rc = opennsl_l3_egress_ecmp_traverse(0, l3ecmp_egress_dynamic_mode_set_reset, NULL);
        if (OPENNSL_FAILURE(rc)) {
            VLOG_ERR("Failed to traverse ECMP groups to reset resilient mode ");
        }
    }
                    ...
                    int intf_count, opennsl_if_t *info, void *user_data)
{
    ...
    opennsl_l3_egress_ecmp_create(0, ecmp, intf_count, egress_obj);

    return 0;
} /*l3ecmp_egress_dynamic_mode_set_reset */
