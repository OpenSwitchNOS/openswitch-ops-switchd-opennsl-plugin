                                ...
                                unsigned char *mac, struct netdev *netdev)
{
    ...
    if (netdev_bcmsdk_get_subint_ref_count(netdev) == 0) {
        handle_bcmsdk_knet_subinterface_filters(netdev, true);
    }
    netdev_bcmsdk_update_subint_ref_count(netdev, true);
                       ...
                       opennsl_l3_intf_t *l3_intf, struct netdev *netdev)
{
    ...
    netdev_bcmsdk_update_subint_ref_count(netdev, false);
...
ops_subinterface_fp_entry_create(opennsl_port_t hw_port, int hw_unit)
{
    ...
    if (l3_fp_grp_info[hw_unit].subint_fp_entry_id == -1) {
        ...
        VLOG_INFO("%s ADD hw_port = %d, added",
                 __FUNCTION__, hw_port);
    } else {

        VLOG_INFO("Entry already created now update the entry = %d, with port = %d",
...
ops_update_subint_fp_entry(int hw_unit, opennsl_port_t hw_port, bool add)
{
    ...
    VLOG_INFO("%s, Get Inport bitmask unit = %d",
    ...
    VLOG_INFO("%s, Get Inport bitmask unit = %d pbm = %s",
    ...
    if (add) {
        ...
        OPENNSL_PBMP_PORT_ADD(pbm, hw_port);
        OPENNSL_PBMP_PORT_ADD(pbm_mask, hw_port);
        VLOG_INFO(" AFTER ADD hw_port = %d, added to pbm = %s",
                 hw_port, _SHR_PBMP_FMT(pbm, pfmt));
    } else {
        /* del hw_port to the rule */
        VLOG_DBG(" DELETE hw_port = %d", hw_port);
    ...
    VLOG_INFO("reinstall entry = %d", l3_fp_grp_info[hw_unit].subint_fp_entry_id);
