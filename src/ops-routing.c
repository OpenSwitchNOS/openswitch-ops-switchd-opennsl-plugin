...
struct ops_l3_feature_fp_info l3_feature_grp_info[MAX_SWITCH_UNITS];
static int
...
failed_vlan_creation:
    ...
    if (OPENNSL_FAILURE(rc)) {
        VLOG_ERR("Failed to delete port to entry = %d for group = %d,\
                Unit=%d port=%d rc=%s",
                l3_feature_grp_info[hw_unit].subint_fp_entry_id,
                l3_feature_grp_info[hw_unit].l3_feature_grpid,
 ...
 */
opennsl_error_t
ops_create_group_l3_feature(int hw_unit)
{
    /* FP groups for subinterface and L3 stats*/
    opennsl_error_t rc = OPENNSL_E_NONE;
    /* Qset for l3 feature */
    opennsl_field_qset_t l3_feature_qset;


    VLOG_INFO("Create group, Check group = %d", l3_feature_grp_info[hw_unit].l3_feature_grpid);
    /* If group is already created then don't create again */
    if (l3_feature_grp_info[hw_unit].l3_feature_grpid == -1) {

        OPENNSL_FIELD_QSET_INIT(l3_feature_qset);
        /* for subinterface */
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset,
                                opennslFieldQualifyMyStationHit);
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyInPorts);

        /* RX counter stats */
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyIpType);
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyL3Ingress);
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyPacketRes);

        /* TX counter stats */
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyIpType);
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyDstPort);
        OPENNSL_FIELD_QSET_ADD (l3_feature_qset, opennslFieldQualifyPacketRes);

        VLOG_DBG("%s, Create FP for unit = %d", __FUNCTION__, hw_unit);
        rc = opennsl_field_group_create(hw_unit, l3_feature_qset,
                                        FP_GROUP_PRIORITY_0,
                                        &l3_feature_grp_info[hw_unit].l3_feature_grpid);
        if (OPENNSL_FAILURE(rc)) {
        ...
        VLOG_DBG("%s, Created FP Group = %d for unit = %d",
                 __FUNCTION__, l3_feature_grp_info[hw_unit].l3_feature_grpid, hw_unit);
    }
...
ops_subinterface_fp_entry_create(opennsl_port_t hw_port, int hw_unit)
{
    ...
    rc = ops_create_group_l3_feature(hw_unit);
    if (OPENNSL_FAILURE(rc)) {
    ...
    if (l3_feature_grp_info[hw_unit].subint_fp_entry_id == -1) {
        ...
        if (OPENNSL_FAILURE(rc)) {
            VLOG_ERR("Failed to set qualify for my station hit \
                    Unit=%d port=%d rc=%s",
                    hw_unit, hw_port, opennsl_errmsg(rc));
            ops_destroy_l3_feature_entry_grp(hw_unit,
            l3_feature_grp_info[hw_unit].subint_fp_entry_id);
            return rc;
        }
        ...
        rc = opennsl_field_entry_install(hw_unit, l3_feature_grp_info[hw_unit].subint_fp_entry_id);
        if (OPENNSL_FAILURE(rc)) {
            VLOG_ERR("Failed to install group = %d entry = %d\
                    ...
                    l3_feature_grp_info[hw_unit].subint_fp_entry_id,
                    hw_unit, hw_port, opennsl_errmsg(rc));
            ops_destroy_l3_feature_entry_grp(hw_unit, l3_feature_grp_info[hw_unit].subint_fp_entry_id);
            return rc;
        }
        VLOG_DBG("%s ADD hw_port = %d, added to subint_count = %d",
                 __FUNCTION__, hw_port,
                 l3_feature_grp_info[hw_unit].subint_count[hw_port]);
        l3_feature_grp_info[hw_unit].subint_count[hw_port]++;
    } else {

        VLOG_DBG("Entry already created now update the entry = %d, with port = %d",
                  l3_feature_grp_info[hw_unit].subint_fp_entry_id, hw_port);
...
ops_update_subint_fp_entry(int hw_unit, opennsl_port_t hw_port, bool add)
{
    ...
    if (add) {
        ...
        VLOG_DBG(" DELETE hw_port = %d, subint_count = %d",
                 hw_port, l3_feature_grp_info[hw_unit].subint_count[hw_port]);
        l3_feature_grp_info[hw_unit].subint_count[hw_port]--;
        if (l3_feature_grp_info[hw_unit].subint_count[hw_port] == 0) {
            ...
            if (OPENNSL_PBMP_IS_NULL(pbm)) {
                VLOG_DBG("pbm is empty so delete the entry = %d",
                        l3_feature_grp_info[hw_unit].subint_fp_entry_id);
                rc = ops_destroy_l3_feature_entry_grp(hw_unit,
                        l3_feature_grp_info[hw_unit].subint_fp_entry_id);
                if (OPENNSL_FAILURE(rc)) {
    ...
    VLOG_DBG("reinstall entry = %d", l3_feature_grp_info[hw_unit].subint_fp_entry_id);
    rc = opennsl_field_entry_reinstall(hw_unit, l3_feature_grp_info[hw_unit].subint_fp_entry_id);
    if (OPENNSL_FAILURE(rc)) {
        VLOG_ERR("Failed to install group = %d entry = %d\
                Unit=%d port=%d rc=%s",
                l3_feature_grp_info[hw_unit].l3_feature_grpid,
...
/*
 * Function deletes the l3_feature group entries.
 * If no entry exists then the group is deleted.
 */
opennsl_error_t
ops_destroy_l3_feature_entry_grp(int hw_unit, opennsl_field_entry_t entryid)
{
    opennsl_error_t rc = OPENNSL_E_NONE;
    int entry_count = 0;
    ...
    VLOG_DBG("%s group = %d entry_count = %d\n",
              __FUNCTION__, l3_feature_grp_info[hw_unit].l3_feature_grpid, entry_count);
...
ops_l3_fp_init(int hw_unit)
{
    ...
    l3_feature_grp_info[hw_unit].subint_fp_entry_id = -1;
    memset(l3_feature_grp_info[hw_unit].subint_count, 0, sizeof(l3_feature_grp_info[hw_unit].subint_count));
    return 0;
}
