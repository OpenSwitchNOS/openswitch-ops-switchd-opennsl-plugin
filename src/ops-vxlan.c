 ...
 *
 * vxlan_port_refcnt:
 *      number of vxlan ports refer to this egress object.
 *
 * node:
 *      hash node
 */
 ...
 *      number of vxlan access ports refer to this physical port
 *
 * vxlan_net_port_refcnt:
 *      number of vxlan network ports refer to this physical port
 *
 * node:
 *      hash node
 */
...
static int vxlan_get_port_refcnt(int unit, int port,
         int *vxlan_acc_port_refcnt, int *vxlan_net_port_refcnt);
static int vxlan_cleanup_port_refcnt(int unit, int port);
 ...
 *      User must check if the port does not
...
vxlan_insert_port_hash_element(int unit, int port)
{
    ...
    port_element_p = (vxlan_port_element_t *)
        calloc(1, sizeof(vxlan_port_element_t));

    if (port_element_p == NULL) {
        VLOG_ERR("Error [%s, %d], port_element_p calloc failed for unit:%d port:%d\n",
                 __FUNCTION__, __LINE__, unit, port);
        return NULL;
    }
...
/*
 * Function: vxlan_cleanup_port_refcnt
 *      Clean up the port reference count when needed
 */
static
int
vxlan_cleanup_port_refcnt(int unit, int port)
{
...
vxlan_access_port_unconfigure(int unit, int port)
{
    ...
    if (vxlan_acc_port_refcnt > 1) {
        ...
        rc = vxlan_cleanup_port_refcnt(unit, port);
        return rc;
    }
