...
bcmsdk_vxlan_endis_global(int unit, int endis)
{
    ...
    if (rc) {
        VLOG_ERR("Error [%s, %d], vxlan_endis rc:%d unit:%d endis:%d\n",
                 __FUNCTION__, __LINE__, rc, unit, endis);
        return rc;
    }

    VLOG_DBG("[%s, %d], vxlan_endis rc:%d unit:%d endis:%d\n",
             __FUNCTION__, __LINE__, rc, unit, endis);
                                      ...
                                      bcmsdk_vxlan_logical_switch_t *logical_sw_p)
{
    ...
    if (logical_sw_p == NULL) {
        VLOG_ERR("Error [%s, %d], bcmsdk_vxlan_logical_switch_operation logical_sw_p is NULL unit:%d opcode:%d\n",
    ...
    VLOG_DBG("[%s, %d], bcmsdk_vxlan_logical_switch_operation rc:%d unit:%d opcode:%d\n",
                              ...
                              bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (tunnel_p == NULL) {
        VLOG_ERR("Error [%s, %d], bcmsdk_vxlan_tunnel_operation tunnel_p is NULL unit:%d opcode:%d\n",
                 __FUNCTION__, __LINE__, unit, opcode);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], bcmsdk_vxlan_tunnel_operation rc:%d unit:%d opcode:%d\n",
             __FUNCTION__, __LINE__, rc, unit, opcode);
                            ...
                            bcmsdk_vxlan_port_t *port_p)
{
    ...
    if (port_p == NULL) {
        VLOG_ERR("Error [%s, %d], bcmsdk_vxlan_port_operation port_p is NULL unit:%d opcode:%d\n",
                 __FUNCTION__, __LINE__, unit, opcode);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], bcmsdk_vxlan_port_operation rc:%d unit:%d opcode:%d\n",
             __FUNCTION__, __LINE__, rc, unit, opcode);
                                 ...
                                 bcmsdk_vxlan_multicast_t *multicast_p)
{
    ...
    if (multicast_p == NULL) {
        VLOG_ERR("Error [%s, %d], bcmsdk_vxlan_multicast_operation multicast_p is NULL unit:%d opcode:%d\n",
                 __FUNCTION__, __LINE__, unit, opcode);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], bcmsdk_vxlan_multicast_operation rc:%d unit:%d opcode:%d\n",
             __FUNCTION__, __LINE__, rc, unit, opcode);
...
vxlan_configure_global(int unit, vxlan_setting_t *vxlan_set_p)
{
    ...
    if (!vxlan_set_p) {
        VLOG_ERR("Error [%s, %d], vxlan_configure_global vxlan_set_p is NULL\n",
    ...
    VLOG_DBG("[%s, %d], vxlan_configure rc:%d unit:%d opennslSwitchL3EgressMode:%d opennslSwitchVxlanUdpDestPortSet:%d opennslSwitchVxlanEntropyEnable:%d opennslSwitchVxlanTunnelToCpu:%d VxlanVnIdMissToCpu:%d\n",
             __FUNCTION__, __LINE__, rc, unit,
...
vxlan_create_logical_switch(int unit, bcmsdk_vxlan_logical_switch_t *logical_sw_p)
{
    ...
    if (!logical_sw_p) {
        VLOG_ERR("Error [%s, %d], vxlan_create_logical_switch logical_sw_p is NULL\n",
                 __FUNCTION__, __LINE__);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d] vxlan_create_logical_switch rc:%d unit:%d vnid:%d broadcast_group:%d unknown_multicast_group:%d unknown_unicast_group:%d vpn:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
             vpn_info.vnid, vpn_info.broadcast_group,
             vpn_info.unknown_multicast_group,
             vpn_info.unknown_unicast_group,
...
vxlan_destroy_logical_switch(int unit, bcmsdk_vxlan_logical_switch_t *logical_sw_p)
{
    ...
    VLOG_DBG("[%s, %d] vxlan_destroy_logical_switch rc:%d unit:%d vnid:%d vpn_id:0x%x\n",
...
vxlan_get_logical_switch(int unit, bcmsdk_vxlan_logical_switch_t *logical_sw_p)
{
    ...
    if (!logical_sw_p) {
        VLOG_ERR("Error [%s, %d], vxlan_get_logical_switch logical_sw_p is NULL\n",
                 __FUNCTION__, __LINE__);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], opennsl_get_logical_switch rc:%d unit:%d vnid:%d broadcast_group:0x%x unknown_multicast_group:0x%x unknown_unicast_group:0x%x vpn:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
...
vxlan_create_tunnel_initiator(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (!tunnel_p) {
        VLOG_ERR("Error [%s, %d], vxlan_create_tunnel_initiator tunnel_p is NULL\n",
                 __FUNCTION__, __LINE__);
        return BCMSDK_E_PARAM;
    }
    ...
    if (rc) {
        VLOG_ERR("Error [%s, %d], opennsl_create_tunnel_initiator rc:%d unit:%d sip:0x%08x dip:0x%08x udp_dst_port:%d udp_src_port:%x ttl:%d\n",
                 __FUNCTION__, __LINE__, rc, unit,
    ...
    VLOG_DBG("[%s, %d], opennsl_create_tunnel_initiator rc:%d unit:%d sip:0x%08x dip:0x%08x udp_dst_port:%d udp_src_port:%x ttl:%d tunnel_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
             tunnel_p->local_ip, tunnel_p->remote_ip,
...
vxlan_destroy_tunnel_initiator(int unit, int tunnel_id)
{
    ...
    if (rc) {
        VLOG_ERR("Error [%s, %d], opennsl_destroy_tunnel_initiator rc:%d unit:%d tunnel_id:%d\n",
                 __FUNCTION__, __LINE__, rc, unit, tunnel_id);
        return rc;
    }

    VLOG_DBG("[%s, %d], opennsl_destroy_tunnel_initiator rc:%d unit:%d tunnel_id:%d\n",
             __FUNCTION__, __LINE__, rc, unit, tunnel_id);
...
vxlan_get_tunnel_initiator(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (!tunnel_p) {
        VLOG_ERR("Error [%s, %d], opennsl_get_tunnel_initiator tunnel_p is NULL\n",
                 __FUNCTION__, __LINE__);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], pennsl_get_tunnel_initiator rc:%d unit:%d sip:0x%08x dip:0x%08x udp_dst_port:%d udp_src_port:%x ttl:%d tunnel_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
             tunnel_p->local_ip, tunnel_p->remote_ip,
             tunnel_p->udp_dst_port, tunnel_p->udp_src_port, tunnel_p->ttl,
             tunnel_p->tunnel_id);
...
vxlan_create_tunnel_terminator(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (!tunnel_p) {
        VLOG_ERR("Error [%s, %d], opennsl_get_tunnel_terminator tunnel_p is NULL\n",
                 __FUNCTION__, __LINE__);
        return BCMSDK_E_PARAM;
    }
    ...
    if (rc) {
        VLOG_ERR("Error [%s, %d], opennsl_create_tunnel_terminator rc:%d unit:%d sip:0x%08x dip:0x%08x tunnel_id:%d flags:0x%x vlan:%d\n",
                 __FUNCTION__, __LINE__, rc, unit, tnl_term.sip, tnl_term.dip,
                 tnl_term.tunnel_id, tnl_term.flags, tnl_term.vlan);
        return rc;
    }

    VLOG_DBG("[%s, %d], opennsl_create_tunnel_terminator rc:%d unit:%d sip:0x%08x dip:0x%08x tunnel_id:%d flags:0x%x vlan:%d\n",
             __FUNCTION__, __LINE__, rc, unit,  tnl_term.sip, tnl_term.dip,
             tnl_term.tunnel_id, tnl_term.flags, tnl_term.vlan);
...
vxlan_destroy_tunnel_terminator(int unit, int tunnel_id)
{
    ...
    if (rc) {
        VLOG_ERR("Error [%s, %d], opennsl_destroy_tunnel_terminator rc:%d unit:%d tunnel_id:%d\n",
                 __FUNCTION__, __LINE__, rc, unit, tunnel_id);
        return rc;
    }

    VLOG_DBG("[%s, %d], opennsl_destroy_tunnel_iniator rc:%d unit:%d tunnel_id:%d\n",
             __FUNCTION__, __LINE__, rc, unit, tunnel_id);
...
vxlan_get_tunnel_terminator(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    VLOG_DBG("[%s, %d], opennsl_get_tunnel_initiator rc:%d unit:%d sip:0x%08x dip:0x%08x udp_dst_port:%d udp_src_port:%x ttl:%d vlan:%d tunnel_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
             tunnel_p->local_ip, tunnel_p->remote_ip,
             tunnel_p->udp_dst_port, tunnel_p->udp_src_port, tunnel_p->ttl,
...
vxlan_update_tunnel_terminator(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (rc) {
        VLOG_ERR("Error [%s, %d], opennsl_vxlan_tunnel_terminator_create rc:%d unit:%d sip:0x%08x dip:0x%08x tunnel_id:%d flags:0x%x vlan:%d\n",
                 __FUNCTION__, __LINE__, rc, unit,
    ...
    VLOG_DBG("[%s, %d], opennsl_update_tunnel_terminator rc:%d unit:%d sip:0x%08x dip:0x%08x tunnel_id:%d flags:0x%x vlan:%d\n",
             __FUNCTION__, __LINE__, rc, unit,  tnl_term.sip, tnl_term.dip,
             tnl_term.tunnel_id, tnl_term.flags, tnl_term.vlan);
...
vxlan_create_tunnel(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (!tunnel_p) {
        VLOG_ERR("Error [%s, %d], vxlan_create_tunnel tunnel_p is NULL\n",
                 __FUNCTION__, __LINE__);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], vxlan_create_tunnel rc:%d unit:%d tunnel_p:%p\n",
             __FUNCTION__, __LINE__, rc, unit, tunnel_p);
...
vxlan_destroy_tunnel(int unit, int tunnel_id)
{
    ...
    VLOG_DBG("[%s, %d], vxlan_destroy_tunnel rc:%d unit:%d tunnel_id:%d\n",
             __FUNCTION__, __LINE__, rc, unit, tunnel_id);
...
vxlan_get_tunnel(int unit, bcmsdk_vxlan_tunnel_t *tunnel_p)
{
    ...
    if (tunnel_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_get_tunnel tunnel_p is NULL unit:%d\n",
...
vxlan_access_port_configure(int unit, int port)
{
    ...
    VLOG_DBG("[%s, %d], vxlan_access_port_configure unit:%d port:0x%x\n",
...
vxlan_bind_access_port(int unit, bcmsdk_vxlan_port_t *acc_port_p)
{
    ...
    if (acc_port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_bind_access_port acc_port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], vxlan_bind_access_vport rc:%d unit:%d port:0x%x l3_intf_id:0x%x egr_obj_id:0x%x vlan:%d vnid:%d local_mac:%s next_hop_mac:%s vxlan_port_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
             acc_port_p->port, acc_port_p->l3_intf_id,
             acc_port_p->egr_obj_id, acc_port_p->vlan,
...
vxlan_unbind_access_port(int unit, bcmsdk_vxlan_port_t *acc_port_p)
{
    ...
    if (acc_port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_unbind_access_port acc_port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], vxlan_unbind_access_vport rc:%d unit:%d port:0x%x port:0x%x vpn_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
...
vxlan_get_access_port(int unit, bcmsdk_vxlan_port_t *acc_port_p)
{
    ...
    if (acc_port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_get_access_port acc_port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], vxlan_get_access_vport rc:%d unit:%d vxlan_port_id:0x%x vlan:%d vnid:%d port:%d\n",
             __FUNCTION__, __LINE__, rc, unit,
...
vxlan_network_port_configure(int unit, int port)
{
    ...
    VLOG_DBG("[%s, %d], vxlan_network_port_configure unit:%d port:0x%x\n",
             __FUNCTION__, __LINE__, unit, port);
...
vxlan_bind_network_port(int unit, bcmsdk_vxlan_port_t *net_port_p)
{
    ...
    if (net_port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_bind_network_port net_port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
    ...
    opennsl_l3_intf_t_init(&l3_intf);
    l3_intf.l3a_flags = OPENNSL_L3_ADD_TO_ARL;
    memcpy(l3_intf.l3a_mac_addr, net_port_p->local_mac,
           ETH_ALEN);
    l3_intf.l3a_vid = net_port_p->vlan;
    l3_intf.l3a_vrf = net_port_p->vrf;
    rc = opennsl_l3_intf_create(unit, &l3_intf);
    if (rc) {
        VLOG_ERR("Error [%s, %d], opennsl_l3_intf_create rc:%d unit:%d local_mac:%02x%02x%02x%02x%02x%02x vlan:%d vrf:%d\n",
                 __FUNCTION__, __LINE__, rc, unit,
        ...
        goto CLEANUP_NETWORK_PORT_CFG;
    }
    net_port_p->l3_intf_id = l3_intf.l3a_intf_id;
    ...
    VLOG_DBG("[%s, %d], vxlan_bind_network_vport rc:%d unit:%d port:0x%x l3_intf_id:0x%x egr_obj_id:0x%x vlan:%d vnid:%d vpn_id:0x%x local_mac:%02x%02x%02x%02x%02x%02x next_hop_mac:%02x%02x%02x%02x%02x%02x station_id:0x%x vxlan_port_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
             net_port_p->port, net_port_p->l3_intf_id,
             net_port_p->egr_obj_id, net_port_p->vlan,
             net_port_p->vnid,
 ...
 CLEANUP_L3_INTF:
    ...
    if (rc1) {
        VLOG_ERR("Error [%s, %d], opennsl_l3_intf_delete rc:%d unit:%d l3_intf_id:0x%x\n",
                 __FUNCTION__, __LINE__, rc1, unit, net_port_p->l3_intf_id);
        return rc;
    }

 CLEANUP_NETWORK_PORT_CFG:
...
vxlan_unbind_network_port(int unit, bcmsdk_vxlan_port_t *net_port_p)
{
    ...
    if (net_port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_unbind_network_port net_port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], vxlan_unbind_network_vport rc:%d unit:%d port:0x%x vxlan_port_id:0x%x vnid:%d vpn_id:0x%x\n",
             __FUNCTION__, __LINE__, rc, unit,
...
vxlan_get_network_port(int unit, bcmsdk_vxlan_port_t *net_port_p)
{
    ...
    if (net_port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_get_network_port net_port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
    ...
    VLOG_DBG("[%s, %d], vxlan_get_network_port rc:%d unit:%d vxlan_port_id:0x%x vlan:%d port:%d vrf:%d vnid:%d vpn_id:0x%x local_mac:%02x%02x%02x%02x%02x%02x next_hop_mac:%02x%02x%02x%02x%02x%02x\n",
             __FUNCTION__, __LINE__, rc, unit,
...
vxlan_bind_port(int unit, bcmsdk_vxlan_port_t *port_p)
{
    ...
    if (port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_bind_port port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
...
vxlan_unbind_port(int unit, bcmsdk_vxlan_port_t *port_p)
{
    ...
    if (port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_unbind_port port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
...
vxlan_get_port(int unit, bcmsdk_vxlan_port_t *port_p)
{
    ...
    if (port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_get_port port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
...
vxlan_update_port(int unit, bcmsdk_vxlan_port_t *port_p)
{
    ...
    if (port_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_update_port port_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
...
vxlan_create_multicast(int unit, bcmsdk_vxlan_multicast_t *multicast_p)
{
    ...
    if (multicast_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_multicast_create multicast_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
...
vxlan_destroy_multicast(int unit, bcmsdk_vxlan_multicast_t *multicast_p)
{
    ...
    if (multicast_p == NULL) {
        VLOG_ERR("Error [%s, %d], vxlan_multicast_destroy multicast_p is NULL unit:%d\n",
                 __FUNCTION__, __LINE__, unit);
        return BCMSDK_E_PARAM;
    }
