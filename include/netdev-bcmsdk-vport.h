...
#define PORT_MIN  0
#define PORT_MAX  52 /* CHECK ME */
#define INVALID_VALUE (-1)
#define VALID_VALUE(val)      (val  != INVALID_VALUE)
#define VALID_PORT(port)      (port > PORT_MIN && port <= PORT_MAX)
 ...
 * rather a preventive care which avoid problem going further
 * to BCM functions
 */
...
void netdev_bcmsdk_vport_set_tunnel_vport (struct netdev *netdev, bcmsdk_vxlan_port_t * vport);
...
const bcmsdk_vport_t * netdev_bcmsdk_vport_get_tunnel_vport (struct netdev *netdev);
const carrier_t * netdev_bcmsdk_vport_get_carrier(struct netdev *netdev);
void netdev_vport_update_host_chg(int event, int port, char *ip_addr, int l3_egress_id);
