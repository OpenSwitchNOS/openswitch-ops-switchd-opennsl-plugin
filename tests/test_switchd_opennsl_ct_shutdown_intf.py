...
def validate_ecmp_check_status(switch, number_of_intf):
    ...
    for curLine in buf.split('\n'):
        if "Interfaces:" in curLine:
            ...
            print "%s" % curLine
	    # checks if local number_of_intf = Interfaces + number_of_intf in ASIC
            if str_tok_len != number_of_intf + 1:
               LogOutput('error', "ASIC for ECMP object has incorrect number of nexthops\n")
               assert(False)

# The validate_route_status validates the route info with sh ip route
def validate_route_status(switch, is_enabled):
...
def intf_shut_noshut_test(**kwargs):
    ...
    validate_route_status(switch, True)
    ...
    retCode = retStruct.returnCode()
    assert retCode == 0, "Unable to enable interface1 on switch1"
    validate_route_status(switch, False)
