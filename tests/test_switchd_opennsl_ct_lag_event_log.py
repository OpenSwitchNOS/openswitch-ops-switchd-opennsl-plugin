...
def lag_event_log_check_status(switch1):
    ...
    # Show LAG event logs
    retStruct = switch1.DeviceInteract(command="show events category LAG")
    retCode = retStruct.get('returnCode')
    assert retCode == 0, "Failed to show LAG event logs on SW1"

    buf = retStruct.get('buffer')
    log = re.findall("Trunk set succeeds",buf)
    if log == "Trunk set succeeds":
        return 0
    else:
        return -1

def lag_event_log_test(**kwargs):
    ...
    lag_event_log_check_status(switch1)

class Test_lag_event_log_ct:

    def setup_class(cls):
        # Test object will parse command line and formulate the env
        Test_lag_event_log_ct.testObj = testEnviron(topoDict=topoDict)
        # Get topology object
        Test_lag_event_log_ct.topoObj = Test_lag_event_log_ct.testObj.topoObjGet()

    def teardown_class(cls):
        Test_lag_event_log_ct.topoObj.terminate_nodes()

    def test_lag_tcpudp_hash_ct(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")

        retValue = lag_event_log_test(switch1=dut01Obj, switch2=dut02Obj)
        if retValue != 0:
            assert "Test failed"
        else:
            LogOutput('info', "\n### Test Passed ###\n")
