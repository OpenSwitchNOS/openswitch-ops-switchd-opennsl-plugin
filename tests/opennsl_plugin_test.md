...
## Test sFlow
### Objective
The test case checks whether sFlow (Sampled Flow) gets successfully configured in the ASIC through OpenSwitch CLI. We verify both the ingress and egress sampling rates and the knet filter ids created each for source and destination samples.
### Requirements
A physical switch is required for this test.
...
knet filter ID should be created for source and destination samples
#### Test fail criteria
Sampling rate or knew filter ID creation fails
